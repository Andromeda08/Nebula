...
namespace Nebula::ndx
{
    ...
    : mWindow(swapchainInfo.window), mBackBufferCount(swapchainInfo.backBufferCount)
    {
        ...
        NDX_CHECK_IF_DEBUG(swapchain.As(&mSwapchain), "Failed to cast IDXGISwapChain1 to IDXGISwapChain3");
        ...
        NDX_CHECK_IF_DEBUG(mRTVHeap->SetName(L"Swapchain Descriptor Heap"), "Failed to name ID3D12DescriptorHeap");
    ...
    std::shared_ptr<DX12Swapchain> DX12Swapchain::createDX12Swapchain(const DX12SwapchainInfo& swapchainInfo, const ComPtr<IDXGIFactory4>& pFactory, const DX12DevicePtr& pDevice)
    {
        return std::make_shared<DX12Swapchain>(swapchainInfo, pFactory, pDevice);
    }

    void DX12Swapchain::present(const uint32_t syncInterval, const uint32_t flags) const
    {
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE DX12Swapchain::getRTVHandle(uint32_t i) const
    {
        ...
        const CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(mRTVHeap->GetCPUDescriptorHandleForHeapStart(), static_cast<int>(i), mRTVSize);
        return rtvHandle;
    }
